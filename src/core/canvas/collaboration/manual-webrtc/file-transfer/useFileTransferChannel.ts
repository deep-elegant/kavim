...
export const useFileTransferChannel = ({
  ...
  const outgoingTransfersRef = useRef<Map<string, OutgoingTransferState>>(
    new Map(),
  );
    ...
    (message: FileAckMessage) => {
      ...
      updateTransfer(message.id, (previous) => {
        ...
        const status: FileTransferStatus =
          progress > 0 && previous.status === "pending"
            ? "in-progress"
            : previous.status;
  ...
  const finalizeIncomingTransfer = useCallback(
    async (state: IncomingTransferState) => {
      ...
      const blob = new Blob(orderedPayloads, {
  ...
  useEffect(() => {
    ...
    const cleanup = () => {
      ...
      nextChannel.removeEventListener(
        "bufferedamountlow",
  ...
  const sendFile = useCallback(
    async (
      ...
      const transferState = createOutgoingTransferState(
        id,
        file,
        chunkSize,
        totalChunks,
