...
const extractAssetInputs = (files?: Record<string, Buffer>): PakAssetInput[] => {
  if (!files) {
    return [];
  }
...
const mergeAssets = (base: PakAssetInput[], incoming: PakAssetInput[]): PakAssetInput[] => {
...
export const addPakEventListeners = () => {
  ...
  ipcMain.handle(PAK_ADD_ASSET_CHANNEL, async (_event, asset: PakAssetInput) => {
    const buffer = upsertPakAsset(asset);
    const normalizedAsset: PakAssetInput = { path: asset.path, data: buffer };
    const existingIndex = previousAssets.findIndex((existing) => existing.path === asset.path);
    if (existingIndex >= 0) {
  ...
  ipcMain.handle(PAK_REMOVE_ASSET_CHANNEL, async (_event, assetPath: string) => {
    const removed = removePakAsset(assetPath);
    if (removed) {
      previousAssets = previousAssets.filter((asset) => asset.path !== assetPath);
    }
    return removed;
  });
